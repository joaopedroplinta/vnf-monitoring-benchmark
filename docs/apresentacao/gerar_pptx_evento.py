...
# -*- coding: utf-8 -*-
"""
Gera a apresentacao de evento do TCC em PPTX editavel — tema ESCURO tech/moderno.
Variante de gerar_pptx.py (pre-banca) com marca/rodape ajustados para o evento
da faculdade e ordem de autoria igual a capa do TCC.
...
Uso:  python gerar_pptx_evento.py
Saida: docs/apresentacao/apresentacao_evento.pptx
"""
...
OUT = os.path.join(BASE, "apresentacao_evento.pptx")
FONT = "Arial"
...
def header(s, kicker, ttl, num):
    ...
    para(tf4, [("TCC · IFPR Campus Pinhais", {"size": 10, "color": MUT})], first=True)
...
para(tf, [("João Pedro dos Santos Henrique Plinta   ·   Rafael Correia Alves", {"size": 15, "color": TXT})], space_after=4)
para(tf, [("Orientador: Prof. Me. Guilherme Werneck de Oliveira", {"size": 13, "color": MUT})], space_after=18)
para(tf, [("Pinhais · 2026", {"size": 12, "color": VERDE, "bold": True})])
...
para(tf, [("Dúvidas ou comentários?", {"size": 20, "color": MUT})], space_after=26)
para(tf, [("João Pedro dos Santos Henrique Plinta   ·   Rafael Correia Alves", {"size": 15, "color": TXT})], space_after=6)
